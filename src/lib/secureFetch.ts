...
const useDirectApi=import.meta.env.VITE_API_MODE==='direct';
if(import.meta.env.PROD&&useDirectApi&&!configuredApiBase)throw new Error('VITE_API_BASE_URL is required when VITE_API_MODE=direct');
export const apiBase=(import.meta.env.PROD&&!useDirectApi?'':configuredApiBase||'http://localhost:8080').replace(/\/$/,'');
