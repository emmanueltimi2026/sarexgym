  ...
  return <main className="grid min-h-[100svh] place-items-center bg-[#f4f5f7] p-3 sm:p-4">
    <section className="max-h-[calc(100svh-1.5rem)] w-full max-w-sm overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-xl sm:max-w-md sm:rounded-3xl">
      ...
      <div className="p-5 text-center sm:p-8">
        ...
        {status === 'success' && result && <><CheckCircle2 className="mx-auto h-11 w-11 text-emerald-500 sm:h-14 sm:w-14"/><h1 className="mt-3 text-xl font-black sm:mt-4 sm:text-2xl">Check-in successful</h1><div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-left sm:mt-6 sm:p-5"><div className="flex items-center gap-3 sm:gap-4"><img src={result.member.photo || '/assets/brand/sarex-logo.png'} alt={`${result.member.firstName} ${result.member.lastName}`} className="h-14 w-14 rounded-xl border border-white bg-white object-cover sm:h-16 sm:w-16 sm:rounded-2xl"/><div className="min-w-0"><strong className="block truncate text-base sm:text-lg">{result.member.firstName} {result.member.lastName}</strong><span className="mt-1 inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-[10px] font-black uppercase text-emerald-700"><IdCard className="h-3.5 w-3.5"/>{result.member.memberId}</span></div></div><dl className="mt-4 grid grid-cols-2 gap-2.5 text-[11px] sm:mt-5 sm:gap-3 sm:text-xs"><div><dt className="text-gray-500">Subscription</dt><dd className="mt-0.5 font-bold text-emerald-700 sm:mt-1">{result.member.membershipStatus || 'Active'}</dd></div><div><dt className="text-gray-500">Plan</dt><dd className="mt-0.5 font-bold sm:mt-1">{result.member.planName}</dd></div><div><dt className="text-gray-500">Check-in time</dt><dd className="mt-0.5 font-bold sm:mt-1">{new Date(result.checked_in_at).toLocaleTimeString('en-NG',{hour:'2-digit',minute:'2-digit'})}</dd></div><div><dt className="text-gray-500">Access</dt><dd className="mt-0.5 font-bold text-emerald-700 sm:mt-1">Approved</dd></div><div className="col-span-2"><dt className="text-gray-500">Membership valid until</dt><dd className="mt-0.5 font-bold sm:mt-1">{new Date(result.member.membershipExpiryDate).toLocaleDateString()}</dd></div></dl></div></>}
