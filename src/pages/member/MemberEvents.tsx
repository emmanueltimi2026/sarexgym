...
export const MemberEvents:React.FC=()=>{
 ...
 const[events,setEvents]=useState<any[]>([]),[message,setMessage]=useState('');
 ...
 const register=async(id:string)=>{setMessage('');const r=await fetch(`${BASE}/api/v1/events/${id}/register`,{method:'POST',credentials:'include'}),b=await r.json().catch(()=>({}));if(!r.ok)return setMessage(b?.error?.message||'Unable to register');if(b.data.authorizationUrl)return location.assign(b.data.authorizationUrl);setMessage('Your place is confirmed.');await load()};
 return <><PaymentReceipt/><AppLayout pageTitle="Upcoming Events" pageSubtitle="Discover SAREX outings and activities, then reserve your place." breadcrumbs={[{label:'Member Portal'},{label:'Events'}]}>
  ...
  <div className="grid gap-5 lg:grid-cols-2">
   {!events.length&&<div className="col-span-full rounded-2xl border border-dashed border-gray-300 bg-white p-12 text-center"><CalendarDays className="mx-auto h-10 w-10 text-gray-300"/><h2 className="mt-4 text-xl font-black">No upcoming events</h2><p className="mt-2 text-sm text-gray-500">Published events will appear here when they become available.</p></div>}
