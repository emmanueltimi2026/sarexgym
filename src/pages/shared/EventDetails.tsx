...
const EventBody:React.FC<{event:any;onBack:()=>void;onRegister?:()=>void;message?:string;registerLabel?:string}>=({event,onBack,onRegister,message,registerLabel})=><div className="mx-auto max-w-5xl">
 ...
 <article className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
  ...
  <div className="p-6 sm:p-8">
   ...
   {onRegister&&<button disabled={event.registration_status==='confirmed'||!registrationOpen(event)} onClick={onRegister} className="mt-8 rounded-xl bg-[#EF1B23] px-6 py-3 text-xs font-black uppercase text-white disabled:bg-gray-400">{registerLabel||(event.registration_status==='confirmed'?'Booking confirmed':event.registration_status==='pending_payment'?'Complete payment':!registrationOpen(event)?'Registration closed':Number(event.price_minor)?'Register and pay':'Reserve my place')}</button>}
  </div>
 </article>
</div>;
...
export const EventDetails:React.FC<{mode:'public'|'portal'}>=({mode})=>{
 ...
 const publicRegister=()=>navigate(`/login?returnTo=${encodeURIComponent(`/member/events/${id}`)}`);
 const content=loading&&!event?<div className="h-96 animate-pulse rounded-2xl bg-white" role="status" aria-label="Loading event details"/>:!event&&error?<div className="rounded-2xl border border-red-200 bg-white p-10 text-center text-sm font-bold text-red-600">{error}</div>:<>{error&&<div role="alert" className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">{error}</div>}<EventBody event={event} message={message} onBack={()=>navigate(portalRoot)} onRegister={isMemberEvent?register:mode==='public'?publicRegister:undefined} registerLabel={mode==='public'?'Register':undefined}/>{isMemberEvent&&<MemberRegistrationPanel event={event}/>} {isAdminEvent&&<RegistrationTable event={event}/>}</>;
