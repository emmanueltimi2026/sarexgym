...
export const StaffCheckIn: React.FC = () => {
  ...
  const [result, setResult] = useState<{ member: Member; success: boolean; message: string } | null>(null);
  ...
  return <AppLayout pageTitle="Reception Check-in" pageSubtitle="Display the permanent reception QR and handle manual check-ins when needed." breadcrumbs={[{ label: 'Staff Portal', path: '/staff/dashboard' }, { label: 'Check-in' }]}>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
      <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-xs">
        ...
        {manualOpen && <div className="mt-2 max-h-64 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-sm">{manualMatches.map(member => <button type="button" key={member.id} disabled={busy} onClick={() => void recordManualCheckIn(member)} className="flex w-full items-center justify-between gap-3 border-b border-gray-100 px-3 py-3 text-left text-xs last:border-0 hover:bg-gray-50 disabled:opacity-50"><span><strong className="block text-sm">{member.firstName} {member.lastName}</strong><span className="font-mono text-gray-500">{member.memberId}</span></span><Badge variant={member.membershipStatus === 'Active' ? 'success' : 'danger'}>{member.membershipStatus}</Badge></button>)}{!manualMatches.length && <p className="p-5 text-center text-xs text-gray-500">No member matches that name or ID.</p>}</div>}
        {result && <div className={`mt-5 rounded-xl border p-4 ${result.success ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'}`}><div className="flex items-start gap-3">{result.success ? <CheckCircle className="h-6 w-6 shrink-0"/> : <XCircle className="h-6 w-6 shrink-0"/>}<div><strong className="block">{result.member.firstName} {result.member.lastName}</strong><p className="mt-1 text-xs">{result.message}</p></div></div></div>}
      </section>
